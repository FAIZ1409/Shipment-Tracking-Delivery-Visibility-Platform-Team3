...
public class AnalyticsServiceImpl implements AnalyticsService {
    ...
    public AdminAnalyticsResponse getAdminAnalytics() {
        ...
        // Time-estimate accuracy: how close actual time tracked to the
        // estimate, averaged only across routes that recorded both figures.
        List<Route> routesWithBothTimes = allRoutes.stream()
                .filter(r -> r.getEstimatedTimeMinutes() != null && r.getActualTimeMinutes() != null)
                .toList();

        Double avgAccuracy = average(routesWithBothTimes.stream()
                .map(this::accuracyPercent)
                .toList());

        List<AdminAnalyticsResponse.RoutePerformance.RoutePerformanceItem> rankedByAccuracy =
                routesWithBothTimes.stream()
                        .sorted(Comparator.comparingDouble(this::accuracyPercent).reversed())
                        .map(this::toRoutePerformanceItem)
                        .toList();

        List<AdminAnalyticsResponse.RoutePerformance.RoutePerformanceItem> bestRoutes =
                rankedByAccuracy.stream().limit(5).toList();

        List<AdminAnalyticsResponse.RoutePerformance.RoutePerformanceItem> worstRoutes =
                rankedByAccuracy.stream()
                        .sorted(Comparator.comparingDouble(
                                (AdminAnalyticsResponse.RoutePerformance.RoutePerformanceItem item) ->
        ...
        return AdminAnalyticsResponse.builder()
                ...
                .routePerformance(AdminAnalyticsResponse.RoutePerformance.builder()
                        ...
                        .timeEstimateAccuracyPercent(avgAccuracy)
                        .bestPerformingRoutes(bestRoutes)
                        .worstPerformingRoutes(worstRoutes)
                        .build())
    ...
    // 100 = actual time matched the estimate exactly; drops toward 0 as the
    // deviation grows. Clipped at 0 so a wildly late/early route doesn't
    // pull an average negative. Caller must ensure both times are non-null.
    private double accuracyPercent(Route route) {
        int estimated = route.getEstimatedTimeMinutes();
        int actual = route.getActualTimeMinutes();

        if (estimated == 0) return actual == 0 ? 100.0 : 0.0;

        double deviationPercent = Math.abs(actual - estimated) * 100.0 / estimated;
        return Math.max(0.0, 100.0 - deviationPercent);
    }

    private AdminAnalyticsResponse.RoutePerformance.RoutePerformanceItem toRoutePerformanceItem(Route route) {
        return AdminAnalyticsResponse.RoutePerformance.RoutePerformanceItem.builder()
                .trackingNumber(route.getShipment() != null ? route.getShipment().getTrackingNumber() : null)
                .origin(route.getOrigin())
                .destination(route.getDestination())
                .distanceKm(route.getDistanceKm())
                .estimatedTimeMinutes(route.getEstimatedTimeMinutes())
                .actualTimeMinutes(route.getActualTimeMinutes())
                .delayMinutes(DeliveryMetrics.delayMinutes(route))
                .accuracyPercent(accuracyPercent(route))
                .build();
    }
